...
struct ContestView: View {
    ...
    private func errorView(_ error: String) -> some View {
        VStack(spacing: 16) {
            ...
            Button("다시 시도") { Task { await loadContestsAsync() } }.buttonStyle(.borderedProminent)
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
    }

    private func loadContestsAsync() async {
        isLoading = true
        errorMessage = nil
        do {
            let response = try await service.fetchContests()
